...
#######################################################################################
#############        set up to push eventbridge logs to cloudwatch       ##############
#######################################################################################

# first create log group
resource "aws_cloudwatch_log_group" "eventbridge_logs" {
  name              = "/aws/events/example-scheduled-lambda"
  retention_in_days = 14
}

#EventBridge writes logs using a CloudWatch Logs resource policy, not an IAM role.

resource "aws_cloudwatch_log_resource_policy" "eventbridge_policy" {
...
#
resource "aws_cloudwatch_event_target" "eventbridge_logs_target" {
  rule      = aws_cloudwatch_event_rule.every_five_minutes.name
